...
// Buat instance Primbon
const primbon = new Primbon();

// Fungsi cek API key (sesuaikan dengan sistem Anda)
...
module.exports = [
    ...
    // Endpoint 2: Tafsir Mimpi
    {
        name: "Tafsir Mimpi",
        desc: "Menafsirkan arti mimpi berdasarkan kata kunci",
        category: "Primbon",
        path: "/primbon/tafsirmimpi?apikey=&mimpi=",
        async run(req, res) {
    ...
    // Endpoint 3: Kecocokan Nama Pasangan
    {
        name: "Kecocokan Nama Pasangan",
        desc: "Menilai kecocokan hubungan berdasarkan nama pasangan",
        category: "Primbon",
        path: "/primbon/jodoh?apikey=&nama1=&nama2=",
        async run(req, res) {
            const { apikey, nama1, nama2 } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!nama1 || !nama2) return res.json({ status: false, error: "Nama1 dan Nama2 wajib diisi" });
    ...
    // Endpoint 4: Tanggal Jadian / Pernikahan
    {
        name: "Tanggal Jadian / Pernikahan",
        desc: "Karakteristik hubungan berdasarkan tanggal jadian atau pernikahan",
        category: "Primbon",
        path: "/primbon/tanggaljadi?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi (format: 1-2-2000)" });

            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
    ...
    // Endpoint 5: Ramalan Jodoh Lengkap
    {
        name: "Ramalan Jodoh Lengkap",
        desc: "Ramalan kecocokan jodoh berdasarkan nama dan tanggal lahir",
        category: "Primbon",
        path: "/primbon/ramalanjodoh?apikey=&nama1=&tanggal1=&nama2=&tanggal2=",
        async run(req, res) {
            const { apikey, nama1, tanggal1, nama2, tanggal2 } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!nama1 || !tanggal1 || !nama2 || !tanggal2) {
                return res.json({ status: false, error: "Semua parameter wajib diisi" });
            }

            const tgl1 = Tanggal(tanggal1);
            const tgl2 = Tanggal(tanggal2);
            try {
                const result = await primbon.ramalan_jodoh(
                    nama1, tgl1.tanggal, tgl1.bulan, tgl1.tahun,
                    nama2, tgl2.tanggal, tgl2.bulan, tgl2.tahun
                );
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
    ...
    // Endpoint 6: Rejeki Weton
    {
        name: "Rejeki Weton",
        desc: "Mengetahui rejeki, hoki, dan grafik keberuntungan weton",
        category: "Primbon",
        path: "/primbon/rejekiweton?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi" });
    ...
    // Endpoint 7: Kecocokan Nama & Tanggal
    {
        name: "Kecocokan Nama & Tanggal",
        desc: "Kecocokan nama berdasarkan tanggal lahir",
        category: "Primbon",
        path: "/primbon/kecocokannama?apikey=&nama=&tanggal=",
        async run(req, res) {
            const { apikey, nama, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!nama || !tanggal) return res.json({ status: false, error: "Nama dan tanggal wajib diisi" });
    ...
    // Endpoint 8: Hari Baik
    {
        name: "Hari Baik",
        desc: "Menentukan hari baik berdasarkan Primbon",
        category: "Primbon",
        path: "/primbon/haribaik?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi" });
            ...
            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
                const result = await primbon.petung_hari_baik(tgl, bln, thn);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
    ...
    // Endpoint 9: Hari Larangan
    {
        name: "Hari Larangan",
        desc: "Mengetahui hari pantangan atau hari buruk",
        category: "Primbon",
        path: "/primbon/harilarangan?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi" });
            ...
            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
                const result = await primbon.hari_sangar_taliwangke(tgl, bln, thn);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
    ...
    // Endpoint 10: Ramalan Cinta
    {
        name: "Ramalan Cinta",
        desc: "Ramalan cinta berdasarkan nama dan tanggal lahir pasangan",
        category: "Primbon",
        path: "/primbon/ramalancinta?apikey=&nama1=&tanggal1=&nama2=&tanggal2=",
        async run(req, res) {
            const { apikey, nama1, tanggal1, nama2, tanggal2 } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!nama1 || !tanggal1 || !nama2 || !tanggal2) {
    ...
    // Endpoint 11: Nomor Hoki
    {
        name: "Nomor Hoki",
        desc: "Mengetahui keberuntungan berdasarkan nomor HP",
        category: "Primbon",
        path: "/primbon/nomerhoki?apikey=&nomor=",
        async run(req, res) {
    ...
    // Endpoint 12: Ramalan Nasib
    {
        name: "Ramalan Nasib",
        desc: "Ramalan nasib berdasarkan tanggal lahir",
        category: "Primbon",
        path: "/primbon/ramalannasib?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi" });
            ...
            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
                const result = await primbon.ramalan_nasib(tgl, bln, thn);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
    ...
    // Endpoint 13: Zodiak
    {
        ...
        category: "Primbon",
        path: "/primbon/zodiak?apikey=&zodiak=",
        async run(req, res) {
    ...
    // Endpoint 15: Ramalan Peruntungan
    {
        name: "Ramalan Peruntungan",
        desc: "Ramalan peruntungan di tahun tertentu",
        category: "Primbon",
        path: "/primbon/ramalanperuntungan?apikey=&nama=&tanggal=&tahun=",
        async run(req, res) {
            const { apikey, nama, tanggal, tahun } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!nama || !tanggal || !tahun) {
                return res.json({ status: false, error: "Nama, tanggal lahir, dan tahun peruntungan wajib diisi" });
            }

            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
                const result = await primbon.ramalan_peruntungan(nama, tgl, bln, thn, tahun);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
    ...
    // Endpoint 16: Weton Jawa
    {
        name: "Weton Jawa",
        desc: "Mengetahui weton Jawa berdasarkan tanggal lahir",
        category: "Primbon",
        path: "/primbon/wetonjawa?apikey=&tanggal=",
        async run(req, res) {
            const { apikey, tanggal } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal wajib diisi" });
            ...
            const { tanggal: tgl, bulan: bln, tahun: thn } = Tanggal(tanggal);
            try {
                const result = await primbon.weton_jawa(tgl, bln, thn);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
    ...
    // Endpoint 17: Masa Subur
    {
        name: "Masa Subur",
        desc: "Menghitung masa subur berdasarkan tanggal menstruasi",
        category: "Primbon",
        path: "/primbon/masasubur?apikey=&tanggal=&siklus=",
        async run(req, res) {
            const { apikey, tanggal, siklus = "28" } = req.query;
            if (!cekApikey(apikey, res)) return;
            if (!tanggal) return res.json({ status: false, error: "Tanggal menstruasi terakhir wajib diisi" });

            const { tanggal: dateday, bulan: datemonth, tahun: dateyear } = Tanggal(tanggal);
            try {
                const result = await primbon.masa_subur(dateday, datemonth, dateyear, siklus);
                res.json(result);
            } catch (error) {
                res.json({ status: false, error: error.message });
            }
